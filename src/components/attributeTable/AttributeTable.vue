...
export default {
  ...
  watch: {
    ...
    features() {
      this.records = this.features.map((feature) => {
  ...
  methods: {
    ...
    highlightInitialFeatureSelectionInTable() {
      const correspondingInteraction = MapInteractionUtil.getSelectInteraction(
        this.map,
        this.layerId
      );
    ...
    activateSelectRowOnMapClick() {
      WguEventBus.$on("map-selectionchange", (lid, featureArray) => {
    ...
    applyColumnMapping() {
      ...
      if (this.layer.get("columnMapping")) {
        for (const [propertyName, displayName] of Object.entries(
          this.layer.get("columnMapping")
        )) {
          headers.push({
            text: displayName,
            value: propertyName,
