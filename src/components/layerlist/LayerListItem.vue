<template>
  <!-- Show layer details -->
  <v-list-group v-if="showDetails" v-model="open" class="text--primary">
    ...
    <v-list-item v-if="showOpacityControl" class="overflow-visible">
      <wgu-layeropacitycontrol :layer="layer" />
    </v-list-item>
    <v-list-item v-if="showLegend">
      <!-- Remarks:
      The legend image item is wrapped by an v-if block to avoid unneccesary image
      requests when the layer item is not expanded.
      -->
      <wgu-layerlegendimage v-if="open" :layer="layer" :mapView="mapView" />
    </v-list-item>
  </v-list-group>

  <!-- Simple layer entry -->
  <v-list-item v-else class="wgu-layerlist-item">
    <v-list-item-action>
      <v-checkbox
        color="secondary"
        hide-details
