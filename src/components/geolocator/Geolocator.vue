<template>
  <span>
    <v-btn
      @click="geolocateUserAndShowMarkerOnMap"
      icon
      color="onprimary"
    ...
    >
      ...
      <v-icon
        v-else-if="this.isGeolocationAPIAvailable && !this.isGeolocationFound"
...
export default {
  ...
  methods: {
    ...
    createAndRemoveExistingLayer(layers, layerId) {
      ...
      layers.remove(
        layers.getArray().filter((layer) => layerId === layer.get("lid"))[0]
      );
      // create new layer
        ...
        navigator.geolocation.getCurrentPosition(
          (position) => {
            if (typeof this.map !== "undefined") {
              ...
              const geolocLayer = this.createAndRemoveExistingLayer(
                this.map.getLayers(),
                "wgu-geolocator-layer"
              );
              geolocLayer
                .getSource()
                .addFeature(new Feature({ geometry: currentPosGeom }));
              ...
              console.error(
                "The map is not defined in the getCurrentPosition callback"
              );
            }
          },
