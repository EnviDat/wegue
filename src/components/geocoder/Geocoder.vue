...
export default {
  ...
  computed: {
    resultItems() {
      ...
      this.trace(
        `computed.resultItems() - cur results len=${this.results.length}`
      );
  ...
  watch: {
    ...
    search(queryStr) {
      ...
      queryStr.length >= this.minChars &&
        queryStr !== this.lastQueryStr &&
    ...
    selected(item) {
      ...
      if (Object.prototype.hasOwnProperty.call(result, "boundingbox")) {
        ...
        const extent = applyTransform(
          result.boundingbox,
          getTransform("EPSG:4326", mapProjection)
        );
  ...
  mounted() {
    ...
    this.geocoderController = new GeocoderController(
      this.provider,
