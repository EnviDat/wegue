  ...
  >
    ...
    <div v-if="!this.showMedia">
      <v-card-title primary-title class="wgu-infoclick-win-title">
        <v-card-text
          v-if="!this.attributeData && !this.coordsData"
          class="no-data"
        >
    ...
    <div v-if="this.showMedia">
      ...
      <v-card-actions>
        ...
        >
          {{
            $t("wgu-infoclick.mediaInfoLinkText") ||
            this.attributeData[mediaInfoLinkUrlProp]
          }}
        </v-btn>
      </v-card-actions>
    </div>
...
export default {
  ...
  methods: {
    ...
    onMapClick(evt) {
      const me = this;
      const featureLayer = me.map.forEachFeatureAtPixel(
        evt.pixel,
        (feature, layer) => {
