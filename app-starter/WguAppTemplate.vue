<template>
  <v-app
    id="wgu-app"
    data-app
    :class="{ 'wgu-app': true, 'wgu-app-embedded': isEmbedded }"
  >
    ...
    <wgu-app-header>
      ...
      <slot
        name="wgu-tb-before-auto-buttons"
        slot="wgu-tb-before-auto-buttons"
      />
    ...
    <wgu-app-sidebar v-if="sidebarWins.length" v-bind="sidebarConfig">
      <template v-for="(moduleWin, index) in sidebarWins">
        <component :is="moduleWin.type" :key="index" v-bind="moduleWin" />
      </template>
    </wgu-app-sidebar>
